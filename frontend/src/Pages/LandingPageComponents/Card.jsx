...
const Card = ({
  ...
  const coursePath = resumeLearning
    ? `/course/${_id}/learn`
    : `/course/${_id}`;

  return (
    <Link to={coursePath} target={resumeLearning ? undefined : "_blank"} rel={resumeLearning ? undefined : "noopener noreferrer"}>
      <Flex
        direction={"column"}
        gap={"5px"}
        borderWidth="1px"
      ...
      >
        ...
        <Flex color={resumeLearning ? "#38A169" : "#0056d2"} fontFamily={"poppins"} gap={2}>
          <Flex alignItems={"center"}>
            <FaGraduationCap />
          </Flex>
          <Box>{resumeLearning ? "Continue Learning" : "Earn a degree"}</Box>
        </Flex>
